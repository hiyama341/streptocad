...
def process_specified_gene_sequences_from_record(
    seq_record: Dseqrecord, specified_locus_tags: list
) -> dict:
    """
...
def validate_range_format(item):
    ...
    assert len(positions) == 2, (
        f"Input format error: {item} should contain exactly two numbers separated by a dash."
    )
    ...
    assert start <= end, (
        f"Input format error: the second number {end} is smaller than the first number {start} in {item}."
    )
...
def check_and_convert_input(input_list):
    ...
    if all(re.fullmatch(r"\d+-\d+", item) for item in input_list):
        ...
        raise AssertionError(
            "Input format error: The input should be either a list of ranges or a list of gene names."
        )
...
def annotate_dseqrecord(dseqrecord, target_dict):
    ...
    for annotation in target_dict:
        for region, positions in annotation.items():
            ...
            if not (0 <= start < len(dseqrecord.seq)) or not (
                0 < end <= len(dseqrecord.seq)
            ):
                raise ValueError(f"Positions for region {region} are out of bounds.")

            feature_location = FeatureLocation(start, end, strand=1)
            feature = SeqFeature(
                feature_location, type="CDS", qualifiers={"locus_tag": [region]}
            )
