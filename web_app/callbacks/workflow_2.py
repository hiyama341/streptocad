...
def register_workflow_2_callbacks(app):
    @app.callback(
        [
            ...
            Output(
                "plasmid-metadata-table_2", "data"
            ),  # Output for plasmid metadata DataTable
            Output(
                "plasmid-metadata-table_2", "columns"
            ),  # Output for plasmid metadata DataTable columns
        ],
        [Input("submit-settings-button_2", "n_clicks")],
        [
            State(
                {"type": "upload-component", "index": "genome-file-2"}, "contents"
            ),  # Corrected ID
            State(
                {"type": "upload-component", "index": "single-vector-2"}, "contents"
            ),  # Corrected ID
            State(
                {"type": "upload-component", "index": "genome-file-2"}, "filename"
            ),  # Corrected ID
            State(
                {"type": "upload-component", "index": "single-vector-2"}, "filename"
            ),  # Corrected ID
    ...
    def run_workflow(
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tempdir:
                ...
                targeting_info = [
                    f"pCRISPR-BEST_{row['locus_tag']}_p{row['sgrna_loc']}"
                ...
                pcr_columns = [
                    {"name": col, "id": col} for col in checking_primers_df.columns
                ]
